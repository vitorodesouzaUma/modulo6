...
miParamGrid = {"criterion": ["gini", "entropy"], "max_depth": [3]}
...
axs[2].scatter(Xpcatest[:, 0], Xpcatest[:, 1], cmap="tab10", s=60, c=ypred)
...
ax = pybaobabdt.drawTree(
    miMejorModelo, features=[str(i) for i in range(64)], size=40, dpi=72
)
...
artists = tree.plot_tree(decision_tree=miMejorModelo, filled=True, rounded=True, fontsize=10)
...
ax.scatter(Xtest_norm_pca[:,0],Xtest_norm_pca[:,1],Xtest_norm_pca[:,2],c=ypred,s=50)
"""
