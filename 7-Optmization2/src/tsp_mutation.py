...
class DynamicSwapMutation(Mutation):
    ...
    def update_mutation_rate(self):
        if self.how == 'linear':
            ...
            self.mutation_rate = max(self.mutation_rate - self.step, self.min_mutation_rate)
        elif self.how == 'expo':
            # Exponential Decay
            ...
            self.current_step += 1 # Increment current step
            try:
                self.mutation_rate = self.min_mutation_rate + (self.max_mutation_rate - self.min_mutation_rate) * 1/math.exp(-self.k * 10^-3 * self.current_step)
            except:
                pass
            # Ensure mutation rate does not fall below the minimum
            self.mutation_rate = max(self.mutation_rate, self.min_mutation_rate)
